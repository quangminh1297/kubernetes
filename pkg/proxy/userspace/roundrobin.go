	...
	for portname := range portsToEndpoints {
		...
		if !exists || state == nil || len(newEndpoints) > 0 {
			...
			for i := range newEndpoints {
				...
				klog.V(0).Infof(" *** Check Conver Pod To NodeName ", ConverPodNameToNodeName)
				klog.V(0).Infof(" *** Check Conver Endpoint To PodName ", ConverEndpointToPodName)
...
func (lb *LoadBalancerRR) OnEndpointsUpdate(oldEndpoints, endpoints *v1.Endpoints) {
	...
	for portname := range portsToEndpoints {
		...
		if !exists || state == nil || len(curEndpoints) != len(newEndpoints) || !slicesEquiv(slice.CopyStrings(curEndpoints), newEndpoints) {
			klog.V(0).Infof(" Setting endpoints for %s to %+v", svcPort, newEndpoints) /*Check svcPort: metric-server only 1 time. - nginx-nodePort*/
